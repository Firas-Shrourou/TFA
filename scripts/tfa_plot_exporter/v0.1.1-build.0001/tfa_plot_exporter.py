...
def _plot_energy_fractions(
    traj: dict,
    summary: dict,
    cosmo: dict[str, float],
    outdir: Path,
) -> str | None:
    """Low-redshift energy density fractions plot (non-gated).

    Returns the plot stem ``"energy_fractions"`` on success, or ``None`` if the
    ``energy_fractions`` block is absent from the summary (run produced by an
    older acoustic_validator version).
    """
    av = summary.get("results", {}).get("acoustic_validator", {})
    ef = av.get("energy_fractions")
    if ef is None:
        return None
    ...
    band  = str(av.get("band", ""))
    H0X   = float(av.get("H0_X_kms", float("nan")))
    z_eq_route = ef.get("z_eq_route")
    z_eq_lcdm  = float(ef["z_eq_lcdm"])
    Op0        = float(ef["Omega_phi_0"])
    Om0_route  = float(ef["Omega_m_0"])

    Z_MAX = 3.0
    mask = (traj["z"] >= 0.0) & (traj["z"] <= Z_MAX)
    z_t  = traj["z"][mask]
    Op_t = traj["Omega_phi"][mask]
    idx  = np.argsort(z_t)
    z_t, Op_t = z_t[idx], Op_t[idx]
    Om_t = 1.0 - Op_t

    Om  = cosmo["Omega_m0"]
    Or  = cosmo["Omega_r0"]
    ODE = cosmo["Omega_DE"]
    z_ref = np.linspace(0.0, Z_MAX, 600)
    E2_ref   = Om * (1 + z_ref)**3 + Or * (1 + z_ref)**4 + ODE
    OmL_ref  = ODE / E2_ref
    Omm_lcdm = Om * (1 + z_ref)**3 / E2_ref

    fig, ax = plt.subplots(figsize=(9, 5.5))
    ax.plot(z_ref, OmL_ref,  color="#999",     lw=1.5, ls="--",
            label=r"$\Omega_\Lambda(z)$ — ΛCDM dark energy")
    ax.plot(z_ref, Omm_lcdm, color="#7fc97f",  lw=1.3, ls=":",
            label=r"$\Omega_m(z)$ — matter (ΛCDM)")
    ax.plot(z_t,   Op_t,     "#1f77b4",        lw=2.2,
            label=r"$\Omega_\phi(z)$ — scalar field (ODE)")
    ax.plot(z_t,   Om_t,     "#2ca02c",        lw=2.0, ls="-.",
            label=r"$\Omega_m(z)$ — matter (route background)")
    ax.axvline(z_eq_lcdm, color="#aaa", lw=1.0, ls=":",
               label=f"equality (ΛCDM)  $z = {z_eq_lcdm:.3f}$")
    if z_eq_route is not None:
        ax.axvline(float(z_eq_route), color="#1f77b4", lw=1.0, ls=":",
                   label=f"equality (route)  $z = {float(z_eq_route):.3f}$")
    ax.text(
        0.03, 0.42,
        f"$\\Omega_\\phi(z=0)\\; = {Op0:.5f}$\n"
        f"$\\Omega_m(z=0) = {Om0_route:.5f}$\n"
        f"sum$\\qquad\\qquad\\; = {Op0 + Om0_route:.5f}$",
    ...
    ax.set_title(
        f"{bid}  —  Low-redshift energy density fractions\n"
        f"[$H_{{0,X}} = {H0X:.2f}$, {band_label(band)}]",
        fontsize=9,
    )
    ax.legend(fontsize=8, loc="center right")
    ax.grid(True, lw=0.4, alpha=0.4)
    fig.tight_layout()
    _save(fig, outdir, "energy_fractions")
...
def run_plot_exporter(run_folder: str | Path) -> tuple[str, str]:
    ...
    try:
        ...
        # Non-gated: energy_fractions (requires energy_fractions block in summary)
        ef_stem = _plot_energy_fractions(traj, summary, cosmo, run_folder)
        if ef_stem is not None:
            plots_written.append(ef_stem)
        else:
            plots_skipped.append("energy_fractions")
        ...
        n = len(plots_written)
        gate_skipped = [s for s in plots_skipped if s in ("H_of_z", "delta_H")]
        ef_skipped   = [s for s in plots_skipped if s == "energy_fractions"]
        skipped_parts = []
        if gate_skipped:
            skipped_parts.append(f"{len(gate_skipped)} skipped (export gate)")
        if ef_skipped:
            skipped_parts.append("energy_fractions skipped (block absent)")
        skipped_note = (", " + "; ".join(skipped_parts)) if skipped_parts else ""
