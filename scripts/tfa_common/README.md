...
|---|---:|---:|---:|---|---|---|
| `0.9.3` | `0001` | `0.1` | `0.0.5` | `v0.9.3-build.0001/` | hub - 7 specialists, adds density + CPL-fidelity validators | approved |
| `0.9.2` | `0001` | `0.1` | `0.0.4` | `v0.9.2-build.0001/` | hub - pins T004 core/acoustic/guard stack | superseded |
| `0.9.1` | `0002` | `0.1` | `0.0.4` | `v0.9.1-build.0002/` | hub - pins T001 specialist builds | superseded |
...
**`0.9.3 build 0001` - density + CPL-fidelity validators (T004 deliverables 5b/5c).**
Adds `tfa_density_validator v0.1.0-build.0001` (CPL-free density-sector
diagnostics: H0 pull vs the DESI reference, energy-budget description, f_DE(z),
thawing markers) and `tfa_cpl_fidelity_validator v0.1.0-build.0001` (CPL
audited-not-adopted: best-fit CPL error report + phantom-crossing audit) as the
sixth and seventh in-process non-fatal specialists. Both are non-gated (they run
for EXCLUDED routes too). Physics verdict unchanged.

**`0.9.2 build 0001` - T004 core/acoustic/guard stack.** Updates `_SPECIALIST_BUILDS`
to pin `tfa_core v0.1.0-build.0001`, `tfa_acoustic_validator v0.1.6-build.0001`,
and `tfa_physics_guard_validator v0.1.5-build.0001`. Shared ODE/settings logic
now comes from `tfa_core`; the acoustic engine uses normalized Option A.
